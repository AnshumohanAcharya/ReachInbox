...
export class QueueService {
    ...
    async addAccountToProcessing(accountId: string) {
        try {
            ...
            console.error(`Error adding account ${accountId} to processing:`, error);
            throw error;
        }
    }
    ...
    async removeAccountFromProcessing(accountId: string) {
        try {
            const repeatableJobs = await this.emailQueue.getRepeatableJobs();
            const job = repeatableJobs.find(job => job.name === `process-emails-${accountId}`);
            
            if (job) {
                await this.emailQueue.removeRepeatableByKey(job.key);
                console.log(`Removed account ${accountId} from processing queue`);
            }
        } catch (error) {
            console.error(`Error removing account ${accountId} from processing:`, error);
            throw error;
        }
    }
